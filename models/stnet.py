...
class STNet(nn.Module):
    def __init__(self, num_channels=3, num_classes=43):
        super(STNet, self).__init__()
        self.conv1 = nn.Conv2d(
            num_channels, 200, kernel_size=7, stride=1, padding=2)
        ...
        self.FC1_[2].bias.data.copy_(torch.tensor(
            [1, 0, 0, 0, 1, 0], dtype=torch.float))
        self.FC2_[2].weight.data.zero_()
        self.FC2_[2].bias.data.copy_(torch.tensor(
            [1, 0, 0, 0, 1, 0], dtype=torch.float))
        self.FC3_[2].weight.data.zero_()
        self.FC3_[2].bias.data.copy_(torch.tensor(
            [1, 0, 0, 0, 1, 0], dtype=torch.float))
